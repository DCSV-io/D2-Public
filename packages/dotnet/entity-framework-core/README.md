...
| [`locks-source-gen/`](locks-source-gen/README.md) — `DcsvIo.D2.AdvisoryLocks.SourceGen` | Roslyn `IIncrementalGenerator` that emits `AdvisoryLocks` into the **owning host/module assembly** from `contracts/advisory-locks/advisory-locks.spec.json`. Enforces per-database key uniqueness at build time. Shared owns the tooling; the domain owns the emitted constants. |
