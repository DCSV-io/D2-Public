...
| monorepo-private PackageId `DcsvIo.D2.Private.Auth.Grpc` (AssemblyName policy A may load as `DcsvIo.D2.Auth.Grpc`) | `bindings.Contains("grpc")` | `internal` | `D2GrpcUserStateKeys.g.cs` (class `D2GrpcUserStateKeys` in `DcsvIo.D2.Auth.Grpc.Interceptors`) |
