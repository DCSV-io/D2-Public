...
import { REDIS_CACHE_METER_NAME } from "../src/index.js";

/**
 * Registers a MeterProvider with an in-memory exporter so tests can
 * assert exact `d2.cache.redis.*` counter values.
 */
...
/**
 * Creates a per-test metric harness bound to {@link REDIS_CACHE_METER_NAME}.
 */
  ...
  return {
    ...
    async counterValue(name: string): Promise<number> {
      ...
      for (const scope of resourceMetrics.scopeMetrics) {
        if (scope.scope.name !== REDIS_CACHE_METER_NAME) {
          continue;
        }
