...
```bash
dotnet add package DcsvIo.D2.DataGovernance.EntityFrameworkCore
```
...
Unit tests live in the `DcsvIo.D2.Tests` DataGovernance EF Core unit suite
and the `DcsvIo.D2.Tests` DataGovernance unit suite. Unit-test contexts are
...
test. Integration tests live in the `DcsvIo.D2.Tests` DataGovernance integration suite
...
- `DcsvIo.D2.DataGovernance.Abstractions` — marker interfaces, attribute, rule, and engine seam
- the shared library index — shared library index and dependency graph
