...
- **`EncryptionDomains`** — from `contracts/encryption-domains/encryption-domains.spec.json`. Hosts may extend with product-only domains outside this package; runtime sealed resolution for messaging goes through `EncryptionDomainModeCatalog` after host bootstrap. Cross-language mirror: `@dcsv-io/d2-encryption-abstractions`.
