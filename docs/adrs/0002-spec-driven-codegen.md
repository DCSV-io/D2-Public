...
Every cross-language constant catalog lives in a JSON spec file under `contracts/<topic>/<topic>.spec.json` (paired with a `schema.json`). Two code generators consume each spec and emit the typed artifacts for each runtime:
...
**On the TypeScript side**, per-topic emitters read the same JSON spec files and emit `.g.ts` files into each consuming package's `src/generated/` directory. Generated files are committed so consumers build without the generator.
...
**Forward-only fixture parity tests** assert per-constant name-and-value equivalence between the .NET emitted catalogs and the TypeScript catalogs, both sourced from the same `contracts/<topic>/` specs. A spec change that updates one runtime's emitter but not the other breaks the parity test.

**Process discipline**:

- Hand-coded spec-mirror types are forbidden in any destination assembly. The only path to typed constants in a destination assembly is through the codegen pipeline.
- Internal spec-mirror DTOs inside `*-source-gen/` csprojs are permitted, because `IsRoslynComponent=true` structurally prevents them from leaking to consumers.
- Hand-edits to generated files (`.g.cs`, `.g.ts`, anything under `Generated/` or `src/generated/`) are a process violation. The fix goes to the generator, the spec, or the pipeline.

## Consequences

**Positive.**

- Structural drift is impossible for any catalog covered by a spec. The literal `"AUTH_TOKEN_INVALID"` — emitted as a .NET constant and as a TypeScript constant — comes from the same `constName` field in `contracts/auth-error-codes/auth-error-codes.spec.json`. Neither runtime can diverge without breaking a parity test or a compile.
...
- Adding a new catalog is low marginal cost once the pattern is internalized: any sibling generator README is a copy-paste checklist; the spec design is the dominant cost.
...
- **The discipline burden is real.** The parity-test requirement must be actively enforced at review. A contributor who hand-edits a `.g.cs` to "just fix the value" creates a silent regression the next codegen / `dotnet build` overwrites.
- **Bootstrap friction for a new catalog.** The author must design the spec + JSON schema, write the .NET Loader/Emitter/Generator, write the TypeScript emitter, wire the consuming csproj, produce and commit the generated output, write emitter tests, write parity tests, and update the registry table in `packages/dotnet/README.md`. This deliberate front-loading only makes sense for catalogs with genuine cross-language consumers.
...
- `packages/dotnet/README.md` — source-generators registry + dependency graph (analyzer nodes shown as dashed arrows into destination assemblies).
- `packages/dotnet/source-gen-shared/core/` — `SpecFile.cs`, `LoadResult.cs`, `EmitDiagnostic.cs` (shared scaffolding wired into every `*-source-gen/` csproj).
- `packages/dotnet/result/envelope-source-gen/D2ResultEnvelopeGenerator.cs` — representative single-target generator.
- `packages/dotnet/source-gen-shared/error-codes-source-gen/ErrorCodesGenerator.cs` — second representative generator.
- `contracts/d2result-envelope/d2result-envelope.spec.json` — representative spec (`constName` + `value` + `doc` per entry).
- TypeScript catalogs under `packages/typescript/**` commit matching generated sources from the same specs.
