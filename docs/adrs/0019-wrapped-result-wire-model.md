...
- `contracts/protos/common/v1/d2_result.proto` — the `D2ResultProto` envelope (with `category` field 4); `contracts/d2result-envelope/d2result-envelope.spec.json` — the JSON envelope field-name spec; `contracts/problem-details/problem-details.spec.json` — the ProblemDetails extension keys (incl. `d2_category`).
- `packages/dotnet/result/grpc/ProtoExtensions.cs` — `ToProto` / `ToD2Result<T>` / `HandleAsync<T>` / `IsTransientGrpcException`, with `SanitizedExceptionRender` PII discipline; `packages/dotnet/result/grpc/README.md`.
- Host-supplied gRPC client codec (`d2ResultToProto` / `d2ResultFromProto` / `handleGrpcCall`) and HTTP ProblemDetails producers under `packages/dotnet/aspnetcore/Internal/` plus host auth middleware.
