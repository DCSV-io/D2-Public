...
| `RequestOrigin.cs`                                                                             | Enum — `Unestablished` / `EdgeInbound` / `CrossProcessHop` / `InProcessModule` / `System`. What KIND of trust boundary produced the current request hop's context. Recomputed locally by the receiving boundary from its own unforgeable transport facts every hop; never propagated. `Unestablished` (the zero member) is the fail-closed default — a capability authority (e.g. KeyCustodian's `WorkloadCapabilityAuthority`) denies when the origin has not been positively established. See package docs for `RequestOrigin` / establishment helpers.                                                                                                                |
...
- [`packages/dotnet/messaging/rabbitmq/README.md`](../../messaging/rabbitmq/README.md) — context propagation across AMQP
