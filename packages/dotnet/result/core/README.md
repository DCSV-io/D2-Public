...
| `Generated/.../ErrorCodes.g.cs`                 | Standardized error code constants — `NOT_FOUND`, `FORBIDDEN`, `VALIDATION_FAILED`, `PARTIAL_SUCCESS`, `IDEMPOTENCY_IN_FLIGHT`, `COULD_NOT_BE_SERIALIZED`, `COULD_NOT_BE_DESERIALIZED`, etc. (15 total). Codegen-emitted by [`DcsvIo.D2.Result.ErrorCodes.SourceGen`](../../source-gen-shared/error-codes-source-gen/README.md) from [`contracts/error-codes/error-codes.spec.json`](../../../../contracts/error-codes/error-codes.spec.json). Same spec drives the TS-side `@dcsv-io/d2-result` `ErrorCodes` catalog (generated sources committed) — cross-language drift is structurally impossible. The class also exposes `IReadOnlyList<string> AllCodes` (spec-order enumeration) and `int GetHttpStatus(string)` (declared HTTP status per code; `500` for unknown).                                                                                                                                                                                                                                                                                         |
| `Generated/.../InputErrorWireShape.g.cs`        | JSON property-name catalog for the `InputError` wire shape — `FIELD = "field"` + `ERRORS = "errors"`. Codegen-emitted by [`DcsvIo.D2.WireShapes.SourceGen`](../../source-gen-shared/wire-shapes-source-gen/README.md) from [`contracts/input-error/input-error.spec.json`](../../../../contracts/input-error/input-error.spec.json) (multi-target dispatch by assembly name). Referenced by `InputError`'s `[JsonPropertyName]` attributes; same spec drives the TS-side `@dcsv-io/d2-result` `InputErrorWireShape` catalog (generated sources committed) — cross-language wire drift on the property names is structurally impossible.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `Generated/.../D2ResultEnvelopeFieldNames.g.cs` | JSON property-name catalog for the D2Result Shape B wire envelope (the BFF gateway response shape every frontend reads) — 8 constants: `SUCCESS`, `DATA`, `MESSAGES`, `INPUT_ERRORS`, `ERROR_CODE`, `TRACE_ID`, `STATUS_CODE`, `CATEGORY`. Codegen-emitted by [`DcsvIo.D2.Result.Envelope.SourceGen`](../envelope-source-gen/README.md) from [`contracts/d2result-envelope/d2result-envelope.spec.json`](../../../../contracts/d2result-envelope/d2result-envelope.spec.json) (single-target dispatch on `DcsvIo.D2.Result`). Referenced by `[JsonPropertyName(D2ResultEnvelopeFieldNames.*)]` attributes on every wire-emitted `D2Result` property. Same spec drives the TS-side `@dcsv-io/d2-result` `D2ResultEnvelopeFieldNames` catalog (generated sources committed); the BFF gateway parser reads via these constants instead of hand-rolled string literals. Cross-language wire drift on the 8 envelope field names is structurally impossible. The class also exposes `IReadOnlyList<string> AllFields` (spec-order enumeration). |
...
`packages/dotnet/tests/Unit/Result/` — adversarial coverage across every public surface. Categories:
