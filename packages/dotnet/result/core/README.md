...
| File                                            | Contents                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `D2Result.cs`                                   | Non-generic base — constructor + properties (`Success`, `Failed`, `Messages`, `InputErrors`, `StatusCode`, `ErrorCode`, `TraceId`, `Category`). Every wire-emitted property carries `[JsonPropertyName(D2ResultEnvelopeFieldNames.*)]` referencing the codegen-emitted catalog so the envelope keys ship as camelCase under ANY `JsonSerializerOptions` — callers do not need to configure `JsonNamingPolicy.CamelCase` for the wire shape to render correctly. `Category` is a typed `ErrorCategory?` (from the zero-dep `DcsvIo.D2.ErrorCodes.Category` leaf), stamped by the spec-derived semantic factories from each code's declared category; it serializes as the snake_case wire string and is omitted from the wire when `null`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `D2Result.Factories.cs`                         | Hand-rolled NON-spec-derived factories on `D2Result` — `Ok`, `Created`, `Fail`, `SomeFound`. The spec-derived semantic failure factories (`NotFound`, `Forbidden`, `Unauthorized`, `ValidationFailed`, `Conflict`, `ServiceUnavailable`, `UnhandledException`, `PayloadTooLarge`, `TooManyRequests`, `Canceled`) are codegen-emitted onto the same partial class — see `Generated/.../D2Result.Factories.g.cs`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `D2Result.Booleans.cs`                          | Hand-rolled NON-code-derived discriminators on `D2Result` — the success/status (`IsOk`, `IsCreated`) + composite (`IsPartialOrMissing`, `IsTransientRetryable`) helpers. The 1:1 per-error-code discriminators (`IsNotFound`, `IsConflict`, etc.) are codegen-emitted onto the same partial class — see `Generated/.../D2Result.Booleans.g.cs`. All discriminators carry `[JsonIgnore]` — they're in-process call-site helpers, never wire fields. The non-generic `Failed` property is also `[JsonIgnore]`'d.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `D2Result.Generic.Factories.cs`                 | Hand-rolled NON-spec-derived generic factories — `Ok`, `Created`, `Fail`, `SomeFound`, `PartialSuccess`, plus `BubbleFail` / `Bubble` for upstream-result propagation. The spec-derived typed failure twins (the `<TData>` `NotFound`/… carrying `default` data) are codegen-emitted — see `Generated/.../D2Result.Generic.Factories.g.cs`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `Generated/.../D2Result.Factories.g.cs`         | Spec-derived CONSTRUCTING semantic failure factories on `partial class D2Result` (`NotFound`/`Forbidden`/`Unauthorized`/`ValidationFailed`/`Conflict`/`ServiceUnavailable`/`UnhandledException`/`PayloadTooLarge`/`TooManyRequests`/`Canceled`). Codegen-emitted by [`DcsvIo.D2.Result.ErrorCodes.SourceGen`](../../source-gen-shared/error-codes-source-gen/README.md) (`FactoryHost.Base`) from `contracts/error-codes/error-codes.spec.json` — `factoryShape` drives the signature, `httpStatus`/`code`/`userMessageKey` drive the body. Behaviorally identical to the previously hand-rolled originals (byte-parity-golden pinned).                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `Generated/.../D2Result.Generic.Factories.g.cs` | The `<TData>` typed twins of the above (carry the `new` modifier + `default` data), on `partial class D2Result<TData>`. Same spec source + generator.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `Generated/.../D2Result.Booleans.g.cs`          | Spec-derived per-error-code boolean discriminators on `partial class D2Result` (`IsNotFound`/`IsConflict`/… — 13 total, one per ErrorCode-keyed code; the serialization codes key none). Same spec source + generator.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `Generated/.../ErrorCodes.g.cs`                 | Standardized error code constants — `NOT_FOUND`, `FORBIDDEN`, `VALIDATION_FAILED`, `PARTIAL_SUCCESS`, `IDEMPOTENCY_IN_FLIGHT`, `COULD_NOT_BE_SERIALIZED`, `COULD_NOT_BE_DESERIALIZED`, etc. (15 total). Codegen-emitted by [`DcsvIo.D2.Result.ErrorCodes.SourceGen`](../../source-gen-shared/error-codes-source-gen/README.md) from [`contracts/error-codes/error-codes.spec.json`](../../../../contracts/error-codes/error-codes.spec.json). Same spec drives the TS-side `@dcsv-io/d2-result` `ErrorCodes` catalog via monorepo-private `private/tools/ts-codegen` (not on public export) — cross-language drift is structurally impossible. The class also exposes `IReadOnlyList<string> AllCodes` (spec-order enumeration) and `int GetHttpStatus(string)` (declared HTTP status per code; `500` for unknown).                                                                                                                                                                                                                                                                                         |
| `Generated/.../InputErrorWireShape.g.cs`        | JSON property-name catalog for the `InputError` wire shape — `FIELD = "field"` + `ERRORS = "errors"`. Codegen-emitted by [`DcsvIo.D2.WireShapes.SourceGen`](../../source-gen-shared/wire-shapes-source-gen/README.md) from [`contracts/input-error/input-error.spec.json`](../../../../contracts/input-error/input-error.spec.json) (multi-target dispatch by assembly name). Referenced by `InputError`'s `[JsonPropertyName]` attributes; same spec drives the TS-side `@dcsv-io/d2-result` `InputErrorWireShape` catalog via monorepo-private `private/tools/ts-codegen` (not on public export) — cross-language wire drift on the property names is structurally impossible.                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `Generated/.../D2ResultEnvelopeFieldNames.g.cs` | JSON property-name catalog for the D2Result Shape B wire envelope (the BFF gateway response shape every frontend reads) — 8 constants: `SUCCESS`, `DATA`, `MESSAGES`, `INPUT_ERRORS`, `ERROR_CODE`, `TRACE_ID`, `STATUS_CODE`, `CATEGORY`. Codegen-emitted by [`DcsvIo.D2.Result.Envelope.SourceGen`](../envelope-source-gen/README.md) from [`contracts/d2result-envelope/d2result-envelope.spec.json`](../../../../contracts/d2result-envelope/d2result-envelope.spec.json) (single-target dispatch on `DcsvIo.D2.Result`). Referenced by `[JsonPropertyName(D2ResultEnvelopeFieldNames.*)]` attributes on every wire-emitted `D2Result` property. Same spec drives the TS-side `@dcsv-io/d2-result` `D2ResultEnvelopeFieldNames` catalog via monorepo-private `private/tools/ts-codegen` (not on public export); the BFF gateway parser reads via these constants instead of hand-rolled string literals. Cross-language wire drift on the 8 envelope field names is structurally impossible. The class also exposes `IReadOnlyList<string> AllFields` (spec-order enumeration). |
...
`public/packages/dotnet/tests/Unit/Result/` — adversarial coverage across every public surface. Categories:
